...
VS_OUTPUT VertSkinning( VS_INPUT i, uniform int iNumBones )
{
    ...
    // hack to make shadows look better
	o.Pos.y -= .2f;
    // normalize normals
    Normal = normalize( vso.vNor );

    // Shade (Ambient + etc.)
    o.Diffuse = float4( MaterialAmbient.xyz + saturate( dot( Normal, normalize(-lhtDir.xyz) ) ) * MaterialDiffuse.xyz, 1.0 );
...
technique ShadowPass
{
    pass p0
    {
        VertexShader =  compile vs_2_0 VertSkinning( 4 );
