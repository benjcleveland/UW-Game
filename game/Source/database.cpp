...
void Database::Animate( double dTimeDelta, double dTime, const D3DXVECTOR3 *cameraFacing )
{
	for( dbContainer::iterator i = m_database.begin(); i != m_database.end(); i++ )
	{
		(*i)->Animate( (float)dTimeDelta, dTime, cameraFacing );
	}
}

void Database::AdvanceTimeAndRender( IDirect3DDevice9* pd3dDevice, D3DXMATRIX* pViewProj, double dTimeDelta, D3DXVECTOR3 *pvEye, bool shadowPass )
{
	for( dbContainer::iterator i = m_database.begin(); i != m_database.end(); i++ )
	{
		//(*i)->AdvanceTime( dTimeDelta, pvEye );
		(*i)->Render( pd3dDevice, pViewProj, shadowPass );
	}
}
...
void Database::Release()
{
    ...
    // FIX ME!! this is a hack to coverup issues to get full screen working
    dbContainer::iterator i = m_database.begin();
	while( i != m_database.end() )
	{	//Destroy object
		delete( *i );
...
void Database::setDebug(bool debug)
{
    for( dbContainer::iterator i = m_database.begin(); i != m_database.end(); i++ )
	{
		(*i)->setDebug( debug );
